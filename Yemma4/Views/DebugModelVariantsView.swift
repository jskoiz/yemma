...
struct DebugModelVariantsView: View {
    ...
    private var statusTitle: String {
        ...
        case .intro:
            return "Selected source needs setup"
        case .downloading:
            return "Downloading selected source"
        case .paused:
            return "Download paused"
        case .preparing:
            return "Finishing local setup"
        case .ready:
            return "Selected source is ready"
        case .failed:
    ...
    private var statusMessageText: String {
        if isSwitchingModelSource {
            return "Yemma is unloading the current bundle and preparing the newly selected repository."
        }
        ...
        case .intro:
            return "This repository is selected, but its local MLX bundle is not on the device yet."
        case .downloading:
            return appSetup.chatStatusDetailText ?? "Yemma is downloading the selected repository now."
        case .paused:
            return appSetup.chatStatusDetailText ?? "Resume setup to finish preparing this repository on the device."
        case .preparing:
    ...
    private var sourceKindText: String {
        modelDownloader.activeModelSource.isCustom ? "Custom source" : "Preset source"
    }
    ...
    private var variantsFooterText: String {
        if supportsLocalModelRuntime {
            return "Switching unloads the active runtime first. Yemma reuses any valid cached bundle for the newly selected repository."
        }
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .top, spacing: 14) {
                VStack(alignment: .leading, spacing: 12) {
                    HStack(spacing: 8) {
                        ...
                        ModelVariantChip(
                            text: sourceKindText,
                            systemImage: modelDownloader.activeModelSource.isCustom ? "link" : "shippingbox",
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text(modelDownloader.activeModelSource.title)
            ...
            Link(destination: modelDownloader.activeModelSource.sourceURL) {
                HStack(spacing: 10) {
                    ...
                    Text("View selected repository on Hugging Face")
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(AppTheme.textPrimary)
    ...
    private var overviewStatusPanelContent: some View {
        ...
        case .intro:
            ModelVariantStatusLine(
                ...
                title: "Download required before this source can load.",
                trailing: Self.formatBytes(appSetup.estimatedDownloadBytes),
                detail: "Use the actions section to start saving this repository locally on the device."
            )
        ...
        case .paused:
            VStack(alignment: .leading, spacing: 12) {
                ...
                ModelVariantStatusLine(
                    ...
                    detail: "Yemma kept the verified partial files for this repository."
                )
            }
        ...
        case .ready:
            ModelVariantStatusLine(
                ...
                title: "Validated bundle stored locally",
                trailing: storedModelSizeText,
                detail: "This repository can be loaded again without downloading."
            )
        ...
        case .failed:
            VStack(alignment: .leading, spacing: 12) {
                ...
                ModelVariantStatusLine(
                    ...
                    title: appSetup.chatRecoveryAction?.title ?? "Select a source again",
    ...
    private var variantsSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            UtilitySection("Switch models") {
                ForEach(Array(Gemma4ModelSource.debugPresetSources.enumerated()), id: \.element.id) { index, source in
                    Button {
                        Task {
                            await switchModelSource(to: source)
                        }
                    } label: {
    ...
    private var statusAction: ModelStatusAction? {
        ...
        if let recoveryAction = appSetup.chatRecoveryAction {
            ...
            case .resumeDownload:
                return ModelStatusAction(
                    ...
                    detail: "Continue downloading the selected model from saved progress.",
                    icon: "play.circle",
                    role: nil,
            ...
            case .retryDownload:
                return ModelStatusAction(
                    title: recoveryAction.title,
                    detail: "Try downloading the selected model again.",
            ...
            case .retryModelLoad:
                return ModelStatusAction(
                    title: recoveryAction.title,
                    detail: "Retry loading the already downloaded model bundle.",
                    icon: "bolt.circle",
                    role: nil,
        ...
        if !modelDownloader.isDownloaded && !modelDownloader.isDownloading {
            return ModelStatusAction(
                ...
                detail: "Start downloading the currently selected repository.",
    ...
    private var customModelRow: some View {
        ...
        let detail = isSelected
            ...
            : "Paste a Hugging Face model URL or owner/repository id."
    ...
    private var customModelSheet: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 18) {
                        Text("Paste a Hugging Face repo URL or `owner/repository` id. Yemma validates the MLX files after download before trying to load the model.")
                        ...
                        Text("Preset examples: `mlx-community/gemma-4-e2b-it-4bit` or `unsloth/gemma-4-E4B-it-UD-MLX-4bit`.")
                            .font(AppTheme.Typography.utilityCaption)
                            .foregroundStyle(AppTheme.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
    ...
    @MainActor
    private func switchModelSource(to source: Gemma4ModelSource) async {

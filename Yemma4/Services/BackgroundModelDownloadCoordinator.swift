...
extension BackgroundModelDownloadCoordinator: URLSessionDownloadDelegate, URLSessionTaskDelegate {
    func urlSession(
        ...
        do {
            ...
            // returns, so a failed move or size mismatch would otherwise leave the
            // file permanently missing with no pending task. Self-heal by clearing
            // any partial destination and re-enqueuing a fresh download for it.
